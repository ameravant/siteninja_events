class Admin::EventRegistrationsController < AdminController
  ...
  def index
    ...
    @registrations = EventRegistration.find_all_by_event_id(@event.id, :include => :transactions)
    approved_registrations = @registrations.reject { |x| !x.approved }
    @registration_people = EventRegistrationPerson.find(:all, :conditions => { :event_registration_id => approved_registrations.collect(&:id) }, :include => :people, :include => :event_price)
  end
...
private
  def add_crumbs
    ...
    #add_breadcrumb @event.title , admin_event_registrations_path(@event)
  end
  def find_event
  ...
  def authorization
    authorize(@permissions['events'], "Event Registration")
  end
end
